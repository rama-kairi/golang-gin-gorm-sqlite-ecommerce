...
func (scq *SubCategoryQuery) QueryProducts() *ProductQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, subcategory.ProductsTable, subcategory.ProductsColumn),
		)
		fromU = sqlgraph.SetNeighbors(scq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (scq *SubCategoryQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
//		GroupBy(subcategory.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.SubCategory.Query().
//		Select(subcategory.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (scq *SubCategoryQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*SubCategory, init func(*SubCategory), assign func(*SubCategory, *Product)) error {
	...
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(subcategory.ProductsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sub_category_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
